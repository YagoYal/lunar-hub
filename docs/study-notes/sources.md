...
- **APOD Archive** — `https://apod.nasa.gov/apod/archivepix.html`. Índice HTML com todos os títulos/datas desde 1995, mais recentes no topo. Usado manualmente (via WebFetch) para achar candidatos recentes e coerentes quando o script de ingestão só retorna o "hoje" da API ou nada relevante — o script `nasa-apod.mjs` de hoje não varre o arquivo histórico, só o dia atual. Cada entrada individual (`ap<YYMMDD>.html`) tem o texto completo, o crédito/copyright da imagem e a URL direta da imagem. **Atenção de licença**: nem toda imagem do APOD é da NASA — várias são de astrofotógrafos individuais (ex. "Image Credit & Copyright: Guy Bardon"), que são copyright do autor, não domínio público; preencher `licenseNote` refletindo isso caso a caso, nunca assumir "Public domain (NASA)" por padrão. **[melhoria futura]** dá para estender `nasa-apod.mjs` para paginar o arquivo histórico em vez de só o dia atual, se o volume de candidatos via API ficar baixo demais.
