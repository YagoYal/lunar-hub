...
**CSP: `script-src` precisou de `'unsafe-inline'`**.
Porquê: Astro embute scripts pequenos sem `import` (nosso `BySourceFilter`, `InstallPrompt`, o registro do Service Worker, e o próprio runtime de hidratação de ilhas) direto no HTML como `<script type="module">` inline, em vez de arquivo externo — só scripts com `import` (ex. o do Hero, que importa `@google/model-viewer`) viram chunk externo. `script-src 'self'` sem `'unsafe-inline'` bloqueia todo inline, quebrando filtro, popup de instalação e o registro do SW em produção — e isso nunca apareceu localmente porque `astro dev` não aplica os headers do `vercel.json` (só a Vercel aplica). Alternativa mais estrita (hash SHA-256 por script, gerado no build) foi descartada por complexidade desproporcional ao risco real: o site não usa `set:html` em lugar nenhum (sem HTML bruto de terceiros renderizado), todo conteúdo passa por escaping padrão do Astro e por revisão humana antes de publicar — risco de XSS via injeção de script é baixo o suficiente pra não justificar a complexidade extra.
